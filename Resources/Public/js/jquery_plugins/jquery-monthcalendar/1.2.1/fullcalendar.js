...
(function($) {
	...
	$.fn.fullCalendar = function(options) {
		...
		var timeFormat = options.timeFormat || 'ga';
			...
			$.data(this, 'fullCalendar', {
				...
				removeAndAddEvents: function(eventId, eventArray) {
					...
					for(var j=0; j < eventArray.length; j++){
						events.push(normalizeEvent(eventArray[j]));
					}
					clearEventElements();
					renderEvents();
				},
				...
				for (var i=0; i<eventMatrix.length; i++) {
					...
					for (var j=0; j<levels.length; j++) {
						...
						for (var k=0; k<segs.length; k++) {
							...
							// **** changes from Mario Matzulla: start****
							element.myClass = event.className;
							Calajax.CalendarView.calendar.subscribeToCalendar(element, event);
							// **** changes from Mario Matzulla: end****
						}
						height += maxh;
						top += maxh;
						...
						if (!dragTD || dragTD == dragStartTD) {
							...
							for (var i=0; i<events.length; i++) {
								if (event == events[i] || typeof event.id != 'undefined' && event.id == events[i].id) {
									...
									events[i].className.push("tempEvent");
								}
							}
