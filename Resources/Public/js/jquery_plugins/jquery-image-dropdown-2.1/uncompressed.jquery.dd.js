...
;(function($) {
	...
	function createTitleDiv() {
		...
		arrow = (arrow==undefined || arrow.length==0 || options.showIcon==false) ? "" : '<img src="'+arrow+'" align="left" /> ';
	...
	function openMe() {
		...
		if($("#"+childid).css("display")=="none") {
			selectedValue = a_array[$("#"+childid +" a.selected").attr("id")].value;
			$(document).bind("keydown", function(event) {
													var keyCode = event.keyCode;											
													if(keyCode==39 || keyCode==40) {
														//move to next
	...
	function checkMethodAndApply() {
		...
		if($("#"+elementid).attr("onchange")!=undefined) {
			var currentSelectedValue = a_array[$("#"+childid +" a.selected").attr("id")].value;
