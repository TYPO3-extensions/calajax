...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      ...
      refresh : function() {
         ...
         this._scrollToHour(this.element.data("startDate").getHours());
      },
      ...
      today : function() {
         this._clearCalendar();
         var now = new Date();
         this._loadCalEvents(now);
         this._scrollToHour(now.getHours());
      },
      ...
      prevWeek : function() {
         ...
         this._scrollToHour(new Date().getHours());
      },
      ...
      gotoDate : function(date) {
      	 this.options.firstDayOfWeek = date.getDay();
         this._clearCalendar();
         this._loadCalEvents(date);
         this._scrollToHour(new Date().getHours());
      },
      ...
      // **** changes from Mario Matzulla: start****/
      // this function is needed to replace a recurring event and its instances
      removeAndAddEvents : function(eventId, events) {
            ...
            for(var i = 0; i < events.length; i++) {
            	var $weekDay = self._findWeekDayForEvent(events[i], self.element.find(".wc-time-slots .wc-day-column-inner"));
	            if($weekDay) {
	                self._renderEvent(events[i], $weekDay);
      ...
      _addDraggableToCalEvent : function(calEvent, $calEvent) {
		 ...
		 $calEvent.draggable({
            handle : ".wc-time",
            containment: self.element.find(".wc-scrollable-grid"),
            revert: 'valid',
            opacity: 0.5,
