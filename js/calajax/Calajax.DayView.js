...
/**
 * Day View Builder Class
 */
Calajax.DayView = function() {
...
Calajax.DayView.prototype = {
	...
	fullCalendarDefinitions : function () {
		...
		return {
			firstDay: Calajax.DayView.calendarDef.firstDayOfWeek,
			editable: true,
			header: false,
			aspectRatio: 16.0,
			/*
			...
			*/
			...
			loading: function(isLoading, view) {
				...
				return Calajax.WeekView.Controller.AllDayLoading( isLoading, view );
			...
			dayClick: function( dayDate, view ) {
				...
				return Calajax.WeekView.Controller.AllDayEventRender( dayDate, view );
	...
	calendarDefinitions : function () {
		...
		return {
			...
			firstDayOfWeek : Calajax.Registry.getdate.getDay(), // 1 = Monday
			...
			dateFormat : Calajax.Registry.format.dayHeaderFormat,
			useShortDayNames : Calajax.Registry.format.dayUseShortDayNames,
			/* Translated options */
			...
			data: Calajax.DayView.loadEvents(),
	...
	build : function() {
		...
		Calajax.DayView.calendarDef = this.calendarDefinitions();
		Calajax.DayView.fullCalendarDef = this.fullCalendarDefinitions();

		/*** Alldday ***/
		Calajax.DayView.fullCalendarDef.placeholder = this.placeHolderId + '_head';	 // See Calajax.WeekView.AllDay object
		...
		jQuery( this.wCalBodyID ).weekCalendar( Calajax.DayView.calendarDef );
	...
	today : function() {
		...
		jQuery( this.wCalBodyID ).weekCalendar( 'gotoDate', now );
...
Calajax.DayView.loadEvents = function() {
	...
	return function(start, end, callback) {
		...
		Calajax.DayView.viewStart = start;
		Calajax.DayView.viewEnd = end;
		...
		Calajax.Event.getEvents(start, end, function(data){
			...
			Calajax.WeekView.AllDay.load( Calajax.DayView );
			callback(events);
		});
	};
}
 ...
 * Extend
 */
jQuery.extend( true, Calajax.DayView.prototype, new Calajax.View() );
