/*!
 * FullCalendar2 v1.3.1
 * http://arshaw.com/FullCalendar2/
 *
 * Use FullCalendar2.css for basic styling.
...
$.fn.FullCalendar2 = function(options) {
	...
	if (typeof options == 'string') {
		var args = Array.prototype.slice.call(arguments, 1),
			res;
		this.each(function() {
			var r = $.data(this, 'FullCalendar2')[options].apply(this, args);
			if (res == undefined) {
				res = r;
			}
		});
	...
	this.each(function() {
		...
		function changeView(v) {
			if (v != viewName) {
				...
				if (viewInstances[v]) {
					...
					view = viewInstances[v] = $.FullCalendar2.views[v](
		...
		var publicMethods = {
			...
			removeAndAddEvents: function(eventId, eventArray) {
				...
				for(var j=0; j < eventArray.length; j++){
					normalizeEvent(eventArray[j],options);
					events.push(eventArray[j]);
				}
				//eventsChanged();
				view.clearEvents();
				view.renderEvents(events);
		...
		$.data(this, 'FullCalendar2', publicMethods);
